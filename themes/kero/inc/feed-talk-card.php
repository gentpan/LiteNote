...
@endphp
<article class="kero-row feed-card feed-talk-card" id="talk-{{ $item->id }}">
    <div class="kero-row-label">
        <span class="kero-row-kind">{{ $isMusicTalk ? 'music' : 'talk' }}</span>
        <span class="kero-row-time">{!! \App\Core\Helper::timeTag($item->publishedAt()) !!}</span>
    </div>
    <div class="kero-row-body">
            <div class="talk-content kero-row-desc">{{ $displayContent }}</div>
