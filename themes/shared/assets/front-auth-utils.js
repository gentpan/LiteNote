(function () {
    ...
    function b64urlToBytes(v) {
        ...
        while (v.length % 4) v += '=';
        return Uint8Array.from(atob(v), function (c) { return c.charCodeAt(0); });
    }
    ...
    function pkJson(res) {
        var t = res.headers.get('content-type') || '';
        if (t.indexOf('application/json') === -1) {
            return res.text().then(function () { throw new Error('Passkey 接口返回非 JSON 响应'); });
        }
        return res.json().then(function (d) {
            if (!res.ok || d.success === false) throw new Error(d.message || d.error || 'Passkey 请求失败');
            return d;
        });
    }
    ...
    function loginWithPasskey() {
        if (!window.PublicKeyCredential || !navigator.credentials) {
            return Promise.reject(new Error('当前浏览器不支持 Passkey'));
        }
        return fetch('/admin/passkey/login-options', { headers: { 'X-Requested-With': 'XMLHttpRequest' }, credentials: 'same-origin' })
            .then(pkJson)
            .then(function (options) {
                var allow = (options.allowCredentials || []).map(function (it) {
                    return { type: it.type || 'public-key', id: b64urlToBytes(it.id) };
                });
            ...
            .then(function (assertion) {
                ...
                return fetch('/admin/passkey/login', {
                    method: 'POST',
                    headers: {
